...
class DeepSeekKGExtractor:
    ...
    def __init__(
        ...
        Args:
            ...
            base_url: API 基础 URL
        """
        # 未显式传入旧版 DeepSeek 密钥时，使用系统设置中的活动服务。
        self.use_active_provider = api_key is None
        ...
        if not self.use_active_provider and not self.api_key:
            print("警告：未设置 DeepSeek API 密钥，请设置 DEEPSEEK_API_KEY 环境变量")

    def _get_client(self):
        """获取或创建 API 客户端（延迟初始化）"""
        if self.use_active_provider:
            # The model factory creates the actual provider per invocation so
            # settings changes take effect without rebuilding the KG builder.
            return self
    ...
    def _extract_from_chunk(
        ...
        Returns:
            提取结果
        """
        from app.knowledge_graph.deepseek_config import ENTITY_EXTRACTION_PROMPT
        ...
        print(f"[DEBUG] 发送知识图谱提取提示词，长度：{len(prompt)}")

        try:
            content = self._complete(
                client,
                "你是一个知识图谱构建专家，负责从文本中提取实体和关系，并以严格的 JSON 格式输出。不要输出任何解释，只输出 JSON 数据。",
                prompt,
                temperature=self.temperature,
                max_tokens=self.max_tokens,
            )
            print(f"[DEBUG] 模型响应长度：{len(content)}")
    ...
    def infer_relations(
        ...
        from app.knowledge_graph.deepseek_config import RELATION_INFERENCE_PROMPT
    ...
    def normalize_entities(
        ...
        from app.knowledge_graph.deepseek_config import ENTITY_NORMALIZATION_PROMPT
