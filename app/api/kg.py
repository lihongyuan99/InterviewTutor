...
async def build_kg_from_session(request: KGBuildRequest):
    ...
    try:
        from app.core import memory as mem
        from app.knowledge_graph.kg_builder import KnowledgeGraphBuilder
...
@router.get("/doc-graph")
async def get_doc_graph(task_id: str):
    """获取文档驱动的知识图谱（含按 task 隔离的点亮状态）。"""
    try:
        from app.knowledge_graph.doc_graph import build_doc_graph
        return build_doc_graph(task_id)
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"构建文档知识图谱失败：{str(e)}")


@router.post("/rebuild-doc-graph")
async def rebuild_doc_graph():
    """重新解析 knowledge/ 目录并重建索引（含向量）。"""
    try:
        from app.knowledge_graph.doc_graph import rebuild_doc_graph as _rebuild
        stats = _rebuild()
        return {"status": "success", "stats": stats}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"重建文档知识图谱失败：{str(e)}")
