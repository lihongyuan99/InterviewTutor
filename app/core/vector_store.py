...
Provides semantic retrieval for conversation history using FAISS + Qwen3 Embedding.

Embedding 客户端与题库检索共用（app/core/embedding_client.py），统一使用
Qwen3-Embedding-0.6B-4bit-DWQ（中文友好，1024 维）。
"""
...
def _get_embeddings():
    """Lazy load embeddings client（Qwen3-Embedding，与题库检索共用）。"""
    global _embeddings
    if _embeddings is None:
        from app.core.embedding_client import Qwen3EmbeddingClient
        _embeddings = Qwen3EmbeddingClient()
    return _embeddings
...
class ConversationVectorStore:
    ...
    def load(self) -> bool:
        ...
        # save_local 实际落盘为 {INDEX_FILE}.faiss 与 {INDEX_FILE}.pkl，
        # 因此需检查带扩展名的文件是否存在，而非裸 INDEX_FILE 路径。
        if not (
            os.path.exists(self.index_path + ".faiss")
            or os.path.exists(self.index_path + ".pkl")
        ):
            return False
    ...
    def search(
        ...
        try:
            ...
            for doc, score in results:
                # score 是 FAISS L2 距离（metric_type=L2），越小越相似。
                # 超过阈值的低相关片段直接丢弃，避免噪声污染 prompt。
...
def rag_retrieve(
    ...
    # 空向量库快速跳过：尚未索引任何会话对时，直接返回空，
    # 避免对 query 做无谓的 embedding 请求（远程 HTTP）。
    if store.vector_store is None and not store.metadata:
        return ""
    ...
    for idx, result in enumerate(results, 1):
        ...
        # score 为 FAISS L2 距离（越小越相似），归一化向量 L2 距离范围约 [0, 2]。
        # 转换为 [0, 1] 的相似度用于展示：distance=0 → similarity=1，distance=2 → similarity=0。
